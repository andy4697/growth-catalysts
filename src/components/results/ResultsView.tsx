...
export function ResultsView({ plan, onStartOver }: ResultsViewProps) {
  ...
  const buildCopyText = () => {
    ...
    const contactLines = (plan.contacts ?? [])
      .map((c, i) => `  ${i + 1}. ${c.name} — ${c.title} at ${c.company}${c.linkedin_url ? `\n     LinkedIn: ${c.linkedin_url}` : ""}`)
      .join("\n");

    return `GTM SPARK — YOUR GO-TO-MARKET PLAN
=====================================
    ...
    <div className="w-full max-w-xl space-y-6 pb-8">
      ...
      >
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Strategy by Gemini · Contacts & Wildcard by Happenstance
        </p>
      </motion.div>
      ...
      {plan.gemini.competitor_analysis && plan.gemini.competitor_analysis.length > 0 && (
        <>
          <CompetitorAnalysisSection analyses={plan.gemini.competitor_analysis} />
          <Separator className="opacity-40" />
        </>
      )}
